...
class PerfResult:
    ...
    Attributes:
        perf_str: Formatted string representing performance
            (e.g., '1.23 ± 0.04 us').
...
def _get_script_path(benchmark_name: str) -> str:
    ...
    base_script = (
        benchmark_name if benchmark_name.endswith(".py") else f"{benchmark_name}.py"
    )
...
def run_benchmark(benchmark_name: str) -> str:
    ...
    try:
        ...
        print(
            f"ERROR: Benchmark {benchmark_name} failed with exit code {e.returncode}",
            file=sys.stderr,
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "benchmarks",
        nargs="+",
        help=("Names of the benchmarks to run (e.g., 'matmul_jax', 'matmul_pallas')."),
    )
    ...
    table = [
        _build_row(label, baseline, remaining_benchmarks, result_by_label_by_benchmark)
        for label in labels
    ]
