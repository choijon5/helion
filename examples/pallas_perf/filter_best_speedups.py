...
input. For each unique benchmark prefix (e.g. ``v7_bfloat16_1024x1024x1024``)
it selects the best (fastest) baseline execution time and the best (fastest)
execution time for each benchmark variant across all tiling / subblocking
configurations.

It then recomputes the speedup and error margins for each variant against the
selected canonical baseline and formats a consolidated summary table.
...
def format_variant_summary(
    ...
    Args:
        ...
        variant_col_idx: Column index containing variant performance
            measurements.
...
def main(argv: collections.abc.Sequence[str]) -> None:
    ...
    Reads lines from standard input, groups benchmark data rows by problem
    prefix, selects the absolute minimum execution time for each column
    variant, and recomputes speedups against the canonical baseline.

    Args:
        argv: Command-line arguments passed to the script (including script
            name).
    """
    if len(argv) > 1:
        sys.exit(
    ...
    col_widths = [len(p) for p in COLUMN_SPLIT_RE.split(sep_line)] if sep_line else []

    rows_by_prefix: dict[str, list[ParsedRow]] = collections.defaultdict(list)
    ...
    for prefix, group in sorted(rows_by_prefix.items()):
        ...
        # Align columns to match table formatting using zip to avoid magic
        # index lookups.
        padded_widths = col_widths + [len(c) for c in out_cols[len(col_widths) :]]
        summary_lines.append(
            "  ".join(
                col_str.ljust(width)
                for col_str, width in zip(out_cols, padded_widths, strict=False)
            )
            + "\n"
        )
