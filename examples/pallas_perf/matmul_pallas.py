"""Hand-written Pallas TPU matmul variant used as the perf baseline.

The kernel functions take Pallas ``MemoryRef`` arguments whose type the Pallas
public API does not expose in a typing-friendly way, so they are annotated as
``Any``. ``# ruff: noqa: ANN401`` below covers all of those.
"""
...
def _mask_tile(val: jax.Array, rem: int, dim: int, is_last: jax.Array) -> jax.Array:
    if rem > 0:
...
def pallas_matmul(
    ...
    *,
    ...
    if n == 1:
        ...
        k_rem = k % bk
        return pl.pallas_call(
            functools.partial(matvec_kernel, nsteps=grid_k, k_rem=k_rem, bm=bm, bk=bk),
            grid_spec=pltpu.PrefetchScalarGridSpec(
                num_scalar_prefetch=0,
                in_specs=[
    ...
    if m == 1:
        ...
        k_rem = k % bk
        return pl.pallas_call(
            functools.partial(vecmat_kernel, nsteps=grid_k, k_rem=k_rem, bn=bn, bk=bk),
            grid_spec=pltpu.PrefetchScalarGridSpec(
                num_scalar_prefetch=0,
                in_specs=[
