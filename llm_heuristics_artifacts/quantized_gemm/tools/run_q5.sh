#!/usr/bin/env bash
# Q5: Exp-1 live score — no-autotune baseline vs per-kernel heuristic.
...
OUT=$REPO/llm_heuristics_artifacts/quantized_gemm/iterations/Q5_exp1
...
ts() { date -Iseconds; }
echo "[$(ts)] === Q5 Exp-1 start ==="
nvidia-smi --query-gpu=index,name,memory.used,utilization.gpu --format=csv

# Baseline arm — effort=none default config, no heuristic path
for k in matmul_bf16_int4 _bf16xint16_gemm nvfp4_matmul; do
    echo "[$(ts)] baseline $k"
    unset HELION_LLM_ROUND0_HEURISTIC_PATH || true
    python llm_heuristics_artifacts/quantized_gemm/tools/run_no_autotune.py \
        --kernel "$k" \
        --arm baseline \
        --shape-grid "$SHAPES" \
        --output-dir "$OUT/baseline" \
        --repeats 3 > "$OUT/logs/${k}_baseline.log" 2>&1
    rc=$?
    if [[ $rc -ne 0 ]]; then
...
# Heuristics arm — per-kernel heuristic file
for pair in \
...
do
    ...
    python llm_heuristics_artifacts/quantized_gemm/tools/run_no_autotune.py \
        ...
        --repeats 3 > "$OUT/logs/${k}_heuristics.log" 2>&1
    rc=$?
    if [[ $rc -ne 0 ]]; then
