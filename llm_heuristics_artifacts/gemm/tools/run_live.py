"""Live benchmark runner for the GEMM hill-climbing loop.
...
Example::
    ...
    # baseline
    python run_live.py --kernel matmul --arm baseline --repeats 3 \
        --shape-grid .../shape_grid.json --output-dir .../N0_live/baseline

    # heuristics with AOT seed
    HELION_LLM_ROUND0_HEURISTIC_PATH=.../heuristic_matmul.py \
    python run_live.py --kernel matmul --arm heuristics --repeats 3 \
...
SUPPORTED_KERNELS = ("matmul", "fp8_gemm")
...
class _SeededLLMGuidedSearch(LLMGuidedSearch):
    """LLMGuidedSearch variant that prepends a heuristic-picked seed config.

    If ``skip_llm_stage`` is True, the LLM round-0 call is replaced with
    an empty response, so round 0 benchmarks only seed configs (default +
    random + optional heuristic). Useful for ablations that isolate the
    heuristic's contribution from the LLM's.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--no-llm",
        action="store_true",
        help="Skip the LLM round-0 call (short-circuit to empty response). "
             "Useful for ablations isolating the heuristic seed from the LLM's "
             "round-0 contribution.",
    )
