...
class BenchmarkWorkerPool:
    """Pool of long-lived ``BenchmarkWorker`` processes. ``map_jobs``
    work-steals across workers for parallel precompile; ``run_one`` routes
    to a single worker for serial benchmark timing; ``warmup`` runs one job
    per worker (one-to-one, not work-steal) for spawn + arg preloading."""
    ...
    def run_one(self, job: Callable[[], _T], timeout: float) -> _T:
        return self.workers[0].run(job, timeout=timeout)

    def map_jobs(
        self, jobs: list[Callable[[], object]], timeout: float
    ) -> list[object]:
        """Work-stealing parallel dispatch. Returns one result per input job
        in input order; exceptions appear in-place as values."""
        if not jobs:
            return []
        ...
        def steal(worker_idx: int) -> None:
            ...
            while True:
                try:
                    ...
                    return
                results[i] = _run_capture(worker, jobs[i], timeout)

        _run_in_parallel(steal, self.num_workers)
        return results

    def warmup(
        self, job_factory: Callable[[], Callable[[], object]], timeout: float
    ) -> list[object]:
        """Run one job on *each* worker concurrently (one-to-one, not
        work-stealing) -- the right shape for spawn + arg preloading.

        Callers should run ``start_all`` from a long-lived thread (the main
        thread of the autotune process) before calling ``warmup``; otherwise
        the workers' ``PR_SET_PDEATHSIG`` anchor is the helper thread that
        spawned them, and the workers receive SIGTERM as soon as warmup's
        helper threads exit."""
        results: list[object] = [None] * self.num_workers

        def run_at(i: int) -> None:
            results[i] = _run_capture(self.workers[i], job_factory(), timeout)

        _run_in_parallel(run_at, self.num_workers)
        return results

    def start_all(self) -> None:
        """Eagerly start every worker process from the calling thread.
        ...
        SIGTERM to the worker. If workers were spawned lazily from
        ``warmup``/``map_jobs``/``run_one`` helper threads, they would die as
        soon as that phase returned and the next phase would have to respawn
        them -- paying the disk-bound ``import torch`` cost again. Pinning
        the spawn to the main thread before the first phase keeps every
        worker alive for the rest of the process."""
