...
def compute_repeat(
    ...
    *,
    ...
    # Activate the output pool for the duration of trial timing so per-call
    # ``torch.empty`` allocations hit the safe buffer cache instead of
    # ``cudaMalloc``. The pool is no-op outside this scope for user kernel
    # calls (unless they set ``HELION_REUSE_OUTPUT_BUFFERS=1``).
    with _pool_active():
