...
def _load_args(path: str) -> Sequence[object]:
    # Cached per-worker so jobs sharing args reuse one disk read. Bounded
    # to ``maxsize=2`` so a process-level pool that handles autotunes for
    # multiple input shapes evicts stale args instead of accumulating
    # per-shape copies.
