...
| bf16 1024×1024×1024 (headline)  | 5.52         | 9.92            | 5.52                   | 6.26                 | **1.797**  | **1.000** ✅ | 0.557      | 0.882           | 1.86           | 33.7%     | **33.7%**     | **compute-bound; Helion matches JAX on-device** (G7-prefetch ✅ — cycle-37 lowering substitutes ``jax.jit(lax.dot_general)`` for ``pl.pallas_call`` on the no-tiling autotuner pick; closes the ~21% kH/J gap to 0.00 — Helion-kernel device us 7.43 → 5.52 us, -25.7%, autotuner picks ``[1024, 1024, 1024] unroll`` and the lowering pass fires) |
...
- **``PallasMatmulNoTilingSeedHeuristic``** / **``PallasMatmulF32NoTilingSeedHeuristic``**
  ...
  fire on the bf16/fp16/f32 1024-cube matmul (``M == K == N == 1024``,
  2D, dtype matched) and seed
  ``Config(block_sizes=[M, K, N], pallas_loop_type='unroll',
  ...
  with ``jax.jit(lax.dot_general)``. Pinned to ``dim == 1024`` only
  to avoid changing the autotuner trajectory on larger square shapes
  (2048³ / 4096³) where probe showed well-tuned tiled configs
  consistently match or beat the no-tiling on-device — bumping the
  cap requires a per-shape ablation (see §5 G7-prefetch entry for
  the deferred ``G7-prefetch-large`` follow-up). Lives in
  ``helion/_compiler/autotuner_heuristics/pallas.py``; shares the
  ...
  headline + the dot_general lowering counter fires); no dedicated
  pin test (the lowering pin tests above exercise the counterpart
...
- **G7-prefetch** _(✅ CLOSED 2026-05-25 cycle 37 — headline device kH/J 0.792 → 1.000 (Helion matches JAX exactly), +26% improvement, well above the 5% gate; +17% on-device kernel speedup measured in isolation via 200-call profiler)_. Landed a Pallas-backend lowering pass that substitutes ``jax.jit(lax.dot_general(...))`` for ``pl.pallas_call(...)`` when the autotuner-picked ``block_sizes`` cover every input dim (the no-tiling single-launch case). The dot_general wrapper is visible to XLA's compilation planner so ``cross_program_prefetch_index=0`` becomes reachable (the ``tpu_custom_call`` opacity that DR#7 Track 4 identified as the blocker is bypassed). Detection lives in ``PallasBackend._detect_matmul_dot_general_lowering`` (``helion/_compiler/backend.py``); the launcher swap is in ``default_pallas_launcher`` / ``default_pallas_pipeline_launcher`` via ``_build_matmul_dot_general_jit_fn`` (``helion/runtime/__init__.py``); a ``PallasMatmulNoTilingSeedHeuristic`` + f32 sibling planted in ``helion/_compiler/autotuner_heuristics/pallas.py`` makes the autotuner consider the no-tiling config on the bf16/f32 1024-cube headline shape so the lowering reaches the final pick under the seeded autotuner. Headline 5-sweep cycle-37 medians: helion-kernel 5.52 us / jax 5.52 us / pallas 9.92 us, autotuner consistently picks ``Config(block_sizes=[1024, 1024, 1024], pallas_loop_type='unroll', ...)`` → ``_PALLAS_MATMUL_DOT_GENERAL_LOWERINGS`` counter fires (verified via pin tests).  See the §5 G7 history table cycle-37 row for the full per-shape verification and large-shape spot-check.
  ...
  **Future reach**: the no-tiling seed is currently pinned to ``dim == 1024`` only to avoid changing the autotuner trajectory on the manager-added 2048³ / 4096³ large rows where probe showed tiled configs are already competitive (clean-state today: bf16 2048³ device kH/J 0.92-0.93 even WITHOUT the lowering). Widening the seed to dim >= 1024 requires a per-shape ablation comparing ``[N, N, N]`` no-tiling vs the best tiled config on each row; queued as a follow-up under ``G7-prefetch-large`` whenever the autotuner picks reflect device_us-aware ranking (G7-autotune-device prereq).
...
their cycle-36 baselines (verified via 3-sweep spot-check). **Next
...
``G7-autotune-device`` to re-rank candidate configs by device_us
instead of single-call us so the 5 device-regression skinny shapes
get a fair shake.
