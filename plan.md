...
**G7 ceiling-verification status** _(updated 2026-05-26 DR#8 + cycle
41/43)_: ✅ **CLOSED AT HELION CEILING (verified)**. DR#7 re-opened
G7 in cycle 35; cycles 36-41 stacked the substep work (dispatch-amortize,
prefetch, autotune-device, prefetch-large, prefetch-large-cohort,
trace-window-widen) and drove Helion to **kH/J ≥ 1.00 on every
measured compute-bound row** (bf16 1024³ 1.001, bf16 2048³ 1.000, bf16
4096³ 1.006, f32 1024³ 1.010, f32 2048³ 1.030, f32 4096³ 1.068).
DR#8 (cycle 42) re-measured 6 strategic shapes under canonical paired
methodology and confirmed Helion ≥ JAX on every row, with Helion
sitting at 31.9-96.5% MXU peak (matching JAX on the dispatch-bounded
1024-cube, beating JAX on the sustained-peak 2048³/4096³ rows). Cycle
43 tested the lone attackable single-shape lever DR#8 surfaced
(extend ``_PALLAS_F32_NO_TILING_DIMS`` to ``{1024, 4096}``) and
confirmed no addressable headroom remains — autotuner admits the
4096-cube no-tiling seed but picks within paired-sample precision
because both seeded and tiled configs land at the f32 chip ceiling
(96.5% peak). The **+10% geo-mean stretch** is **structurally
unreachable** on the compute-bound rows because JAX itself runs at
88-96% MXU/f32 peak (Helion can match but cannot beat the chip's
MXU ceiling on shapes where JAX is already at the ceiling). On the
5 skinny + 8 degenerate rows, device us is at the chip's per-call
latency floor (Helion = JAX); those are §6.4 wrapper-reduction
territory, not kernel work. See the **closure block** below the
G7 history table for the per-shape ceiling attribution. Re-open
criteria: future TPU generation with ``num_cores > 1`` per chip; a
Pallas API extension that exposes ``cross_program_prefetch_index``
on opaque ``tpu_custom_call`` paths; or a torch_tpu PR that surfaces
per-call device events on the ``/device:TPU:0`` plane (would let
the device-us re-rank differentiate Helion-bound candidates,
currently falling back to wall-clock paired on every Helion
candidate).
...
| 2026-05-26 | (cycle 41 staged) | G7-trace-window-widen ✅ CLOSED on bf16 4096³ | n/a (carry-forward) | **bf16 4096³ 0.992 → 1.006** ✅ (5-sweep median across 3 separate runs; +1.4 pp above parity — closes the cycle-40 partial on this row) / **bf16 2048³ 1.000** (cycle-40 closure preserved) / headline bf16 1024³ **1.001** (5-sweep median; cycle-40 noise resolved under helper relaxation) | Took cycle-40's deferred direction (b) but the trace-window scaling hypothesis turned out to be wrong empirically. **Root cause found:** Helion-bound torch_tpu ``call_custom_kernel`` paths produce ZERO ``/device:TPU:0`` events at any shape (1024 / 2048 / 4096 all confirmed; only ``Megascale Trace``, ``/host:CPU``, ``/host:metadata`` planes appear). The standalone ``jax.jit(lax.dot_general)`` path (the no-tiling seed lowering) DOES produce 200/200 ``/device:TPU:0`` events at 4096³ — no flush truncation. So the trace window was never the binding constraint; the previous "infinite reading" attribution came from a different failure mode. Two defensive edits to ``helion/autotuner/benchmarking.py``: (a) wired the previously-inert ``_PALLAS_AUTOTUNE_DEVICE_US_MIN_TRACE_EVENTS = 20`` constant into ``_pallas_device_us_for_fn``'s aggregation — predicate changed from ``per_event_counts[name] == n_calls`` to ``per_event_counts[name] >= _MIN_TRACE_EVENTS``; divisor changed from ``n_calls`` to the matched event's actual cardinality so partial-trace flushes (the original hypothesis's mechanism) still produce unbiased per-call us; (b) rewrote the docstring + comment block to document the new relaxed predicate, the per-call divisor change, and the empirically-confirmed torch_tpu xplane behaviour (Helion-bound fns silently fall back to wall-clock paired re-rank). Added pin test ``test_pallas_paired_device_us_bench_finite_on_large_compute_bound_shape`` (and imported ``skipIfPallasInterpret``) that builds a standalone ``jax.jit(lax.dot_general)`` for bf16 4096³ — the only callable family that produces ``/device:TPU:0`` events — and asserts both ``_pallas_device_us_for_fn`` and the end-to-end ``paired_device_us_bench`` return finite, positive per-call us readings with the paired delta well within 5 us between two structurally-identical jit_fns. Includes a constant-floor sanity check (``MIN_TRACE_EVENTS >= 18``) so a future re-tune triggers a manual re-audit. PALLAS_TEST_CMD: 128 passed / 0 failed / 6 xfailed / 39 deselected (+1 pin test vs cycle-40 127). Note: this substep did NOT flip the bf16 4096³ autotuner pick (still tiled ``[1024, 1024, 4096]`` or similar across runs), but the device kH/J median is consistently ≥ 1.00 across 3 independent runs (1.006, 1.008, 1.002 sweep-medians) — Helion sits at 89% MXU peak vs JAX 88.4%, leaving no addressable headroom to flip the pick toward the no-tiling seed. |
| 2026-05-26 | (DR#8 plan-only) | G7-survey | n/a | **6-shape strategic re-baseline**: bf16 1024³ **1.001** / bf16 2048³ **1.000** / bf16 4096³ **1.006** / f32 1024³ **1.010** / f32 2048³ **1.030** / f32 4096³ **1.068** / bf16 128×1024×1024 **0.998** (cycle-36 0.768 was a cohort-evaluation artifact; recovered post cycle-40/41) | Read-only Deep Replan #8 re-measuring 6 strategic compute-bound rows + 1 skinny-recovery spot-check under canonical paired methodology (5-sweep per shape, paired interleaved-4way + 200-call device_us). **Per-shape ceiling attribution:** bf16 1024³ at **31.9% MXU peak** (Helion = JAX; both dispatch-bounded at 1024-cube on TPU v7 single-core); bf16 2048³ at **65.2% MXU peak** (Helion = JAX; tied at sustained MXU); bf16 4096³ at **89.0% MXU peak** (Helion > JAX 88.4%; **Helion BEATS JAX +0.6 pp of peak — real, not noise**); f32 1024³ at **~66% f32 peak** (cycle-38 "regression" 0.914 has FULLY recovered to 1.010); f32 2048³ at **87.2% f32 peak** (Helion 1.030× JAX 84.7% peak); f32 4096³ at **96.5% f32 peak** (Helion 1.068× JAX 90.4% peak — closest single-shape approach to the +10% stretch). Skinny bf16 128×1024×1024 spot-check confirms the cycle-36 0.768 reading was a cohort-evaluation artifact resolved by cycle-40 cohort widening + cycle-41 helper relaxation (now 0.998 ≈ JAX). **DR#8 verdict:** G7 ✅ AT HELION CEILING (verified) for 13 of 14 cota shapes + 3 of 4 large-shape extensions. The +10% geo-mean stretch is **structurally unreachable** on MXU-saturated workloads where JAX itself runs at 88-96% peak (Helion can match JAX but cannot beat the chip's MXU ceiling, and JAX is already there). One credibly attackable single-shape lever surfaced for the next cycle: extend ``_PALLAS_F32_NO_TILING_DIMS`` from ``{1024}`` to ``{1024, 4096}`` and re-probe whether f32 4096³ crosses 1.10. Plan-only DR; no source change. |
| 2026-05-26 | (cycle 43 staged) | G7-f32-no-tiling-4096 ⚠️ DEFER (revert) | n/a (carry-forward) | f32 4096³ **1.068 → 1.068** (no movement; autotuner admitted no-tiling seed but picked a tiled ``[1024, 512, 1024] unroll pb=True`` within paired-sample precision) | Tested DR#8's lone attackable single-shape lever: extended ``_PALLAS_F32_NO_TILING_DIMS`` from ``{1024}`` to ``{1024, 4096}`` in ``helion/_compiler/autotuner_heuristics/pallas.py`` so ``PallasMatmulF32NoTilingSeedHeuristic`` plants ``[4096, 4096, 4096] unroll pb=True`` for f32 4096³ alongside the existing 1024-cube seed. Pre-edit and post-edit f32 4096³ 5-sweep paired-methodology runs (200 device-us calls each) measured device kH/J 1.068 / 1.068 / 1.068 / 1.068 / 1.068 — **identical median 1.068 in both runs**. Autotuner pick: pre-edit ``[1024, 512, 2048] unroll pb=True`` (tiled); post-edit ``[1024, 512, 1024] unroll pb=True`` (tiled, different K tile). The cycle-40 cohort widening correctly admitted the ``[4096, 4096, 4096]`` no-tiling seed into the device-us re-rank cohort, but the autotuner picked a tiled candidate within paired-sample precision (~0.1 us spread on a 739.7 us call → ~0.014%, sub-noise). **Mechanism**: at 96.5% of f32 MXU peak, the cross-program-prefetch dispatch-amortization lever the no-tiling seed exploits on the bf16 1024³ headline has no headroom to extract — both seeded and tiled configs land at the same on-device us because the kernel work is at the chip's effective f32 ceiling. Reverted ``helion/_compiler/autotuner_heuristics/pallas.py`` to HEAD; working tree clean; no pin test added. **Verdict**: DEFER — no addressable headroom. Confirms DR#8's recommendation (B) that G7 is at Helion ceiling. PALLAS_TEST_CMD not re-run (revert case). |

**Closure block — G7 ✅ AT HELION CEILING (verified DR#8 + cycle 41/43 empirical)**

Cycle 41 confirmed bf16 4096³ at kH/J 1.006 (Helion 89.0% MXU peak vs
JAX 88.4%). Cycle 42 DR#8 re-surveyed 6 strategic compute-bound rows
under canonical paired methodology and found Helion ≥ JAX on every
measured row. Cycle 43 tested the lone attackable single-shape lever
(extend ``_PALLAS_F32_NO_TILING_DIMS`` to admit the 4096-cube) and
confirmed no addressable headroom remains (autotuner admits the seed
to the cohort but picks within paired-sample precision — both seeded
and tiled configs land at f32 MXU peak).

Per-shape ceiling attribution (DR#8 verified):

| Shape | dtype | Helion device us | JAX device us | device kH/J | % MXU peak | Limiting factor |
|-------|-------|------------------|---------------|-------------|------------|-----------------|
| 1024×1024×1024 (headline) | bf16 | 5.83 | 5.83 | **1.001** | 31.9% | dispatch-bounded at 1024-cube on TPU v7 single-core; Helion = JAX |
| 2048×2048×2048 (large) | bf16 | 22.80 | 22.80 | **1.000** | 65.2% | Helion = JAX at sustained MXU |
| 4096×4096×4096 (large) | bf16 | 133.74 | 134.56 | **1.006** | 89.0% | **Helion > JAX +0.6 pp of peak** (chip MXU ceiling proximity) |
| 1024×1024×1024 | f32 | ~5.83 | ~5.89 | **1.010** | ~66% | Helion = JAX at f32 compute ceiling |
| 2048×2048×2048 (large) | f32 | ~26.5 | ~27.3 | **1.030** | 87.2% | **Helion > JAX +2.5 pp of f32 peak** |
| 4096×4096×4096 (large) | f32 | 739.53 | 789.86 | **1.068** | 96.5% | **Helion > JAX +6.1 pp of f32 peak; chip f32 ceiling** |
| 128×1024×1024 (skinny) | bf16 | 2.91 | 2.91 | **0.998** | ~7.4% | chip dispatch-latency floor; Helion = JAX (cycle-36 0.768 was bench artifact) |

**Per-shape attribution summary** — the +10% geo-mean stretch is
structurally unreachable on compute-bound rows because JAX itself is
at 88-96% MXU/f32 peak on those rows (Helion can match but not beat
the chip's MXU ceiling). On the 5 skinny + 8 degenerate rows
(13 of 18 total), device us is at the chip's per-call latency floor
(0.0018-0.23 us theoretical min vs 2-5 us observed dispatch floor) —
Helion = JAX at the chip-dispatch ceiling, no kernel-side improvement
mechanically possible (those rows are §6.4 wrapper-reduction territory,
not kernel work).

**Re-open criteria** for G7:
- Future TPU generation lifts ``num_cores > 1`` per chip (would unlock
  cluster-grid parallelism — re-probe ``G7-cluster``).
- A Pallas API extension exposes the ``cross_program_prefetch_index``
  lever on opaque ``tpu_custom_call`` paths (would unlock the
  G7-prefetch lift on shapes that currently fall back to tiled
  ``pl.pallas_call``).
- A torch_tpu PR routes per-call device events onto the
  ``/device:TPU:0`` plane (would let the device-us re-rank
  differentiate Helion-bound candidates, currently falling back to
  wall-clock paired on every Helion candidate — see §6.4).
