...
| **bf16 2048×2048×2048 (large)** | 22.93        | 65.25           | 24.52                  | 24.74                | **2.660**  | 0.935      | 0.351      | 0.927           | 14.87          | 64.9%     | 60.7%         | **compute-bound; mild improvement vs cycle-38 within autotune-noise; G7-prefetch-large no-tiling seed planted (cycle 39) but device-us re-rank top-K picks ``[1024, 1024, 2048]`` paired-delta -0.275us over ``[512, 2048, 2048]`` +0.058us; the ``[2048, 2048, 2048]`` no-tiling seed is in the candidate pool but never reached the top-K cohort** (cycle-38 0.922 → cycle-39 0.935; +1.4 pp, still below the 0.95 acceptance bar — G7-launch-fusion verification target, see §5 G7 cycle-39 DEFER row) |
| **bf16 4096×4096×4096 (large)** | 157.38       | 484.26          | 159.08                 | 158.85               | **3.044**  | **0.989**  | 0.325      | 0.991           | 119.00         | 75.6%     | 74.8%         | **compute-bound-near-peak; mild regression vs cycle-38 within autotune-noise; G7-prefetch-large no-tiling seed planted (cycle 39) but device-us re-rank fell back to wall-clock paired (jax.profiler returned no finite readings for the 4096³ cohort within the trace window), autotuner picked ``[1024, 512, 4096] outer_grid pb=True`` over the ``[4096, 4096, 4096]`` no-tiling seed** (cycle-38 1.002 → cycle-39 0.989; -1.3 pp drift within chip-thermal band, does NOT clear the 1.05 extension bar — G7-launch-fusion verification target, see §5 G7 cycle-39 DEFER row) |
